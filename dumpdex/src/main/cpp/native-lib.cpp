...
jobjectArray dumpDexByCookie(JNIEnv *env, jclass thiz, jlongArray cookie,jstring jdumpDir) {
...
//            env->SetByteArrayRegion(dex_byte, 0, dex_file->data_size_,reinterpret_cast<const jbyte *>(dex_file->begin_));
...
bool GetDexFileByArtMethod(ArtMethod* artMethod,uint32_t code_item_len){
    ...
    for(auto dex_fd : dex_fd_maps){
        ...
        if( dex_begin < code_item_addr &&  code_item_addr <  dex_end){
            ...
            close(dumpdex_fd);
            LOGV("fix code_item successful code_item_len = %d  writ_off = %d ",code_item_len,writ_off);
            return true;
        }
    }
    return false;
...
void dumpMethod(JNIEnv *env, jclass cls, jobject method){
    ...
    if (LIKELY(code_item != nullptr)) {
        ...
        GetDexFileByArtMethod(artMethod,code_item_len);
...
JNIEXPORT jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    jclass classTest = env->FindClass("com/hepta/dumpdex/LoadEntry");
    JNINativeMethod methods[]= {
            ...
            {"dumpDexByCookie", "([JLjava/lang/String;)[Ljava/lang/Object;", (void*)dumpDexByCookie},
    };
